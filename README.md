...
#### New Features
* 为表单添加 提交成功的信息（用户提交表单成功后立即显示，3 秒后会根据提交成功后的行为进行跳转或显示相关的内容）
* 标记字段限制为最多显示 3 个

#### Resolved Issues
* 修复级联选择不能创建的问题（前端问题）
* 管理员修改用户属性时，不校验自定义属性
* 用户通过短信链接查看表单时，如果用户没有填写过该表单，则直接跳转到表单填写页面
* 尝试解决长时间未进入系统导致的 403，404 的错误：当用户访问时尝试自动登录
...
#### New Features
* 可以为空间成员添加自定义属性
* 短信里面添加短链接（需要在空间设置里面打开）
* 禁止用户分享，如果消息不是公开或者不允许匿名填写
* 优化推送
